...
- **Routing** (`src/App.tsx`): `/` → `Home`, `/b2b-platform` → `B2BProject`, `/sub-projects` → `SubProjects`, `/:slug` → `Home` with a per-job-posting variant applied (see below). `src/pages/Pilot.tsx` exists but is **not** registered in the router — it's not part of the live site.
- **Job-posting variants (`src/variants.ts`)**: `VARIANTS` maps a slug to overrides (hero copy, project button copy, section `order`/`hidden`) for `Home`. Visiting `/<slug>` renders `Home` with that variant merged over the defaults; any slug not registered in `VARIANTS` redirects to `/`. These links aren't exposed anywhere in the nav or default pages — only someone given the exact URL can reach them. To make a new per-company link for a job application, add a slug entry to `VARIANTS`.
