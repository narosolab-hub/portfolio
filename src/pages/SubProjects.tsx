  ...
  return (
    <div>
      ...
      <Tile
        ...
        subcopy="오프라인 영업 확대로 표준바코드(코리아넷) 관리가 필요해졌지만, 온라인 중심 사업 구조상 사내 시스템에는 임시 바코드만 등록되어 있었습니다. 수기 처리하기엔 번거로운 작업이 많아 간소화 툴을 만들어 3시간 넘게 걸리던 대조 작업을 10분 내외로 줄여 월 100여 건을 정기적으로 처리하고 있습니다."
      >
        <ProjectMeta
          items={[
            { k: "프로젝트 기간", v: "1주" },
        ...
        <AsIsList
          rows={[
            ...
            {
              k: "리소스 제약",
              v: "사내 IT개발팀 역시 다른 프로젝트 진행 중으로 AI(Claude·Gemini)를 활용해 직접 개발",
            },
          ]}
        />
        <PhaseLead>직접 만든 매칭 툴로 반복되던 수작업을 간소화시켰습니다.</PhaseLead>
        <div className="sub-project__section">
          <SectionLabel>설계 판단</SectionLabel>
          <AsIsList
            rows={[
              { k: "오타 검출", v: "유사코드 자동 추천 (예: S2500960 vs S25006900)" },
              {
                k: "패턴 누적",
                v: "상품코드의 패턴을 파악해 매칭률을 높이고, 검증된 패턴은 보존하고 신규 패턴만 누적하는 방식으로 전환",
              },
            ]}
          />
        </div>
...
송출 불가 제약을 채널 분리로 우회한 신규 세일즈 채널 검증"
        subcopy="226만이 이용하는 인천 시민 지역몰의 이커머스 활성화와 매출 확대를 위해 라이브커머스 시범 운영이 필요했습니다. 시범 운영이었기에 목표는 큰 개발 공수 없이 실제 구매까지 이어지는 라이브커머스 운영이 가능할지 검증하는 것이었습니다. 결과적으로 앱인앱 제약을 해소하기 위해 유튜브 채널을 송출 채널로 채택하여 4회 방송 3,000만원 매출을 달성했습니다."
      >
        <ProjectMeta
          items={[
            { k: "프로젝트 기간", v: "1개월" },
...
반복 VoC를 핵심 재화 접근성 문제로 재정의해 IA/UXUI 전면 개편"
        subcopy="임직원이 지급 받은 복지포인트로 상품을 구매하는 B2B 복지몰 플랫폼입니다. 사용자 대다수가 모바일 앱을 이용했지만 핵심 기능인 복지포인트 현황은 모바일 웹에서만 노출되는 구조였습니다. 마이페이지 개편으로 접근성 문제를 해결하여 정식 런칭 전에는 주 1회에 이상 꾸준히 인입되던 복지포인트 관련 문의가 런칭 후 0건으로 줄었습니다."
      >
        <ProjectMeta
          items={[
