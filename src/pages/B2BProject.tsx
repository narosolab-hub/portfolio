  ...
  return (
    <div>
      ...
      <Tile
        ...
        footer="자사 직접 판매(1P)와 공급사 입점 상품 판매(3P)가 섞여 운영 기준 자체를 재설계했습니다."
      ...
      <Tile
        ...
        subcopy="자사는 플랫폼 운영 주체이자 판매자였기에 두 역할의 권한을 명확히 분리해야 했습니다. 자사 상품을 관리자 예외로 두면 신규 브랜드 런칭마다 운영·정산 예외 처리가 반복돼 개발 공수가 커지는 구조였습니다. 때문에 자사도 외부 공급사와 같은 Seller 모델에 태우고, 자사에만 필요한 수수료 0%·B2B 등급 할인만 정책 예외로 분리했습니다."
      ...
      <Tile
        ...
        subcopy="실제 거래처의 주문 경험을 보았을 때, 위탁배송 특성상 배송지가 일정하지 않았고 상위 거래처는 하루 20~30건의 배송지별 주문을 반복했습니다. (기존 거래처 기준 월평균 약 4,500건) 장바구니는 상품을 담는 시점부터 배송지를 선택하고 여러 배송지를 한 번에 결제하는 구조가 되어야 했으며, 내부에서는 공급사와 배송지 기준으로 주문을 다시 나누었습니다."
      ...
      <Tile
        ...
        subcopy="장바구니에서 일괄 결제는 필요했지만 이후 시스템 안에서는 주문이 하나로 묶이면 운영이 어려웠습니다. 같은 결제 안에서도 공급사·배송지·출고지·묶음배송 여부가 다르고, 상품별로 부분 출고나 취소, 클레임도 발생하기 때문입니다.
초기에는 [결제 및 배송지] 혹은 [결제 및 개별 상품]으로 나누는 2단계 구조를 검토했지만 각각 배송비·정산·부분 처리 기준에서 한계가 있었습니다. 최종적으로는 결제 기준인 ORD, 배송비와 정산 기준인 GRP, 발주와 클레임 기준인 ITM으로 주문 단위를 나누었습니다."
      ...
      <Tile
        ...
        subcopy="자사 상품과 입점사 상품은 판매 주체가 다르기 때문에 일괄 결제 이후 서로 다른 규칙이 필요했습니다. 통신판매중개업 구조상 입점 상품의 결제대금을 자사 계좌로 직접 수취하면 전자금융거래법에 위반되는 법적 리스크도 있었습니다.
이에 1P는 기존 거래 규모를 고려해 후불결제(외상거래)를 제한적으로 유지하고 3P는 PG를 통한 결제대행 및 지급대행 구조로 자사가 판매대금을 직접 수취하지 않도록 분리했습니다."
      >
        <PolicySplitCompare />
      </Tile>
      ...
      <Tile
        ...
        subcopy="앞서 1P와 3P의 결제·정산 기준은 나눈 후에는 매출 신고에 대한 문제가 있었습니다. PG 계약은 자사 명의로 진행되어 국세청 매출 신고는 자사 사업자를 기준으로 잡혔기 때문에 3P 판매대금 전체가 자사 매출이 아니라 공급사에게 정산해야 할 금액과 자사 수수료 매출로 나뉜다는 점을 소명해야 했습니다. 이 부분은 기존 1P 직접 판매 정산만 해오던 재무팀에도 새로운 영역이라 현재 재무팀과 협의하며 전문 회계 검토가 필요한 과제로 정리하고 있습니다."
...
유저 시나리오를 기준으로 판단했을 때, 지금 상태로는 실제 거래를 안정적으로 운영하기 어렵다고 판단해 바로 오픈보다 Phase 2에서 커머스 기능을 보강한 뒤 베타로 오픈하는 방향을 제안했습니다. 현재는 Phase 2 개발이 진행 중입니다."
      ...
      <Tile
        ...
        subcopy="134건의 이슈라이징을 하며 가장 큰 문제였던 케이스입니다. 정산 QA를 진행하며 정산 산정 기준, 부분반품 처리, 상태값 분리, 원 단위 절사 같은 지점들이 화면마다 다르게 처리되거나 아예 기준 없이 개발되었다는 걸 확인했습니다. 실거래가 쌓이면 회계 오차가 반복적으로 누적될 수 있는 지점으로 오픈 전 개발사와 로직을 하나씩 확인하며 정산 기준부터 다시 정리했습니다."
